...
export class ArtiusGoogleProvider extends ArtiusBaseProvider {
  ...
  private async handleFunctionCallIfNeeded(
    ...
    if (response.functionCall?.length > 0) {
      const functionCallResult = this.handleFunctionCall(response, tools ?? {});
      const { contents, config } = await this.prepareRequest({prompt:""}, generationConfig, options, tools, history, functionCallResult);
      console.log(contents);
      const followUp = await this.generateCore(contents, config);
      return normalizeChatResponse(followUp, options);
    }
    return response;
  }
